    ...
    card.innerHTML = `
      <div class="card-info">
        ...
        <div class="player-stat-mini">
          <div class="stat-pill">1ºsrv <span>${Math.round(p.stats.serve1pct * 100)}%</span></div>
...
function renderResult(p1, p2, setsP1, setsP2, winner, scoreStr, stats, matchLog, maxSets) {
  ...
  const rows = [
    ...
    ['Dobles faltas',         stats.doubleFaults[0],    stats.doubleFaults[1]],
    ['Winners',               stats.winners[0],         stats.winners[1]],
    ['Errores no forzados',   stats.errors[0],          stats.errors[1]],
    ['Breaks convertidos',    `${stats.breakConverted[0]}/${stats.breakPoints[0]}`, `${stats.breakConverted[1]}/${stats.breakPoints[1]}`],
    ['Juegos ganados',        stats.totalGames[0],      stats.totalGames[1]],
    ['Puntos 0–4 golpes',     stats.rallyShort[0],      stats.rallyShort[1]],
    ['Puntos 5–8 golpes',     stats.rallyMid[0],        stats.rallyMid[1]],
    ['Puntos 9+ golpes',      stats.rallyLong[0],       stats.rallyLong[1]],
  ];
...
function mobileSelectPlayer(p) {
  if (mobileStep === 0) {
    ...
    if (sel.p1 && p.id === sel.p1.id) return; // mismo jugador
    sel.p2 = p;
    mobileStep = 2;
  }
  updateMobileUI();
...
function updateMobileUI() {
  ...
  // Header de estado
  if (mobileStep === 0) {
    header.innerHTML = '<span class="mob-step-label p1">ELIGE JUGADOR 1</span>';
    ...
    header.innerHTML = `
      ...
      <span class="mob-step-label p2">ELIGE JUGADOR 2</span>`;
    vsBar.style.display = 'none';
  } else {
    header.innerHTML = '';
...
function randomFlashAndSim() {
  ...
  const interval = setInterval(() => {
    ...
    if (count > 10) {
      ...
      setTimeout(() => {
        overlay.remove();
        // Elegir 2 jugadores random distintos
        const pool = [...PLAYERS];
...
function initMobileSimulator() {
  ...
  // Ocultar grids desktop, mostrar móvil
...
function showPlayerDetail(playerId) {
  ...
  document.getElementById('detail-content').innerHTML = `
    ...
    <div class="sim-stats-section">
      <div class="section-label">Velocidades de golpeo</div>
      <div class="speed-cards">
        <div class="speed-card">
          <div class="speed-icon">🎯</div><div class="speed-label">Saque máx.</div>
        ...
        <div class="speed-card">
          <div class="speed-icon">👊</div><div class="speed-label">Derecha</div>
        ...
        <div class="speed-card">
          <div class="speed-icon">🔄</div><div class="speed-label">Revés</div>
        ...
        <div class="speed-card">
          <div class="speed-icon">🏓</div><div class="speed-label">Resto punta</div>
      ...
      <div class="section-label" style="margin-top:2rem">Estadísticas de simulación</div>
      <div class="stat-cards">
        ...
        <div class="stat-card">
          <div class="stat-card-label">Pts con 1er srv</div>
        ...
        <div class="stat-card">
          <div class="stat-card-label">% Devolución</div>
          <div class="stat-card-value">${Math.round(s.returnWin * 100)}%</div>
          <div class="stat-card-sub">Pts ganados devol.</div>
        </div>
        <div class="stat-card">
        ...
        <div class="stat-card">
          ...
          <div class="stat-card-sub">Cuando tiene opción</div>
        </div>
        <div class="stat-card">
        ...
        <div class="stat-card highlight">
          <div class="stat-card-label">Tiebreaks ganados</div>
          <div class="stat-card-value" style="color:var(--accent2)">${Math.round((p.tb_win || 0.62) * 100)}%</div>
          <div class="stat-card-sub">Presión máxima</div>
        </div>
        <div class="stat-card">
      ...
      <div class="surface-section">
        <div class="surface-title">Modificadores por superficie</div>
        <div class="surface-row">
          <div class="surface-label">🔵 Dura</div>
        ...
        <div class="surface-row">
          <div class="surface-label">🟠 Tierra</div>
        ...
        <div class="surface-row">
          <div class="surface-label">🟢 Hierba</div>
      ...
      <div class="surface-section">
        ...
        <div class="rally-distribution">
          ...
          <div class="rally-bar-row">
            ...
            <div class="rally-desc">Intercambios medios, construcción del punto</div>
          </div>
          <div class="rally-bar-row">
            ...
            <div class="rally-desc">Rallies largos, resistencia, físico</div>
          </div>
        </div>
      </div>

      <div class="stat-table">
        <div class="stat-table-header"><div>Estadística</div><div>Valor</div><div>Percentil</div></div>
        ${[
          ['🎯 Vel. saque',        (s.serve_kmh||'—')+' km/h',        barW(s.serve_kmh||200, 170, 250)],
          ['👊 Vel. derecha',      (s.fh_kmh||'—')+' km/h',           barW(s.fh_kmh||145, 118, 178)],
          ['🔄 Vel. revés',        (s.bh_kmh||'—')+' km/h',           barW(s.bh_kmh||138, 115, 162)],
          ...
          ['Devolución',           Math.round(s.returnWin*100)+'%',    barW(s.returnWin, 0.28, 0.52)],
...
function renderComparadorComparison() {
  ...
  const statRows = [
    ...
    ['Devolución',       Math.round(s1.returnWin*100)+'%', Math.round(s2.returnWin*100)+'%', s1.returnWin,      s2.returnWin,      false],
  ...
  document.getElementById('comp-detail-content').innerHTML = `
    ...
    <div class="comp-titles-section">
      <div class="comp-titles-row comp-titles-header">
        ...
        <div class="comp-titles-label">PALMARÉS</div>
        <div class="comp-titles-val"></div>
      </div>
      ...
      <div class="comp-titles-row total-row">
        ...
        <div class="comp-titles-label">TOTAL TÍTULOS</div>
    ...
    <div class="comp-surface-section">
      <div class="section-label">Ventaja por superficie</div>
      ${[['🔵 Dura','hard'],['🟠 Tierra','clay'],['🟢 Hierba','grass']].map(([label, key]) => `
    ...
    <button class="comp-clear-btn" onclick="clearComparador()">↩ Limpiar selección</button>`;
}
...
// HISTÓRICO
// =====================================================
const SURF_LABEL  = { H:'Dura', C:'Tierra', G:'Hierba', I:'Indoor', P:'Hierba', Y:'Hierba' };
...
function applyResultsFilter() {
  ...
  if (countEl) countEl.textContent = `${resultsCurrent.length.toLocaleString()} partidos encontrados`;
}
...
function initializeUI() {
  ...
  console.log('[TL] ✓ Ready — ' + PLAYERS.length + ' jugadores cargados');
}
